...
async def save_group(bot, message):
    ...
    if temp.ME in r_j_check:
        if not await db.get_chat(message.chat.id):
            ...
            await bot.send_message(LOG_CHANNEL, script.LOG_TEXT_G.format(message.chat.title, message.chat.members_count, message.chat.id, total, r_j))       
        ...
        for u in message.new_chat_members:
            if (temp.MELCOW).get('welcome') is not None:
                try:
                    ...
                    pass
            temp.MELCOW['welcome'] = await message.reply_voice("https://hybridf2l.herokuapp.com/143/KERI-VADA-MAKKALE_320k.ogg",
...
@Client.on_message(filters.command('dsong'))
def song(client, message):
    ...
    try:
        ...
        print(str(e))
        return
    m.edit("This May Take Some Time\n**Downloading the song by @malayayalies**")
    try:
        with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            info_dict = ydl.extract_info(link, download=False)
            audio_file = ydl.prepare_filename(info_dict)
            ydl.process_info(info_dict)
        rep = '**Uploaded** by @malayayalies.**'
